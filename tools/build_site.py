...
<ol class="how-list">
  <li><b>You type.</b> Typos, "it" and gene names are fine.</li>
  <li><b>Context.</b> What is open and selected, the documentation of your ChimeraX version indexed locally, 131 tutorial workflows, 58 recipes, a list of known model mistakes.</li>
  <li><b>The model plans.</b> Ollama on your PC, Gemini, Claude or OpenAI. It cannot type into ChimeraX; it can only call tools.</li>
  <li><b>Tools.</b> Run commands, read state, search docs, UniProt lookup, annotate, compare, your tables. Every call is logged.</li>
  <li><b>Safety gate.</b> Closing, deleting, saving and scripts stop for an editable OK card.</li>
  <li><b>ChimeraX runs it.</b> Pellaeon reads the log; an error goes back with the real syntax and a suggestion, and the model retries a bounded number of times.</li>
  <li><b>Reply.</b> Cards with every command: re-run, copy, open its docs. Export the chat as a .cxc script.</li>
</ol>
...
def index_page():
    body = INDEX % {"installer": REPO + "/releases/latest/download/install_pellaeon.py", "repo": REPO, "svg": HOW_SVG}
